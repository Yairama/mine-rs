...
La corrida principal histórica documentada en el reporte local `multi-mine-scheduling-report.json` queda en `664,161,466.99`, con clasificación explícita `exploratory-local`, y por tanto está aproximadamente **25.0% por debajo** de la referencia PCPSP pública (`datasets/benchmarks/outputs/multi-mine-scheduling-report.json:249-253`, `datasets/benchmarks/outputs/multi-mine-scheduling-report.json:1383-1438`). Ese reporte generado no está rastreado por Git; para los números vivos debe usarse `literature-parity.md` y regenerarse la evidencia local cuando corresponda. El problema ya no es solo "mejorar un heurístico": el repo todavía mezcla **gap algorítmico real** con **gap de comparabilidad bibliográfica/protocolar**.
