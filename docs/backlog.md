...
| 180 | MR-180 | `[ ]` | P2 | Planning | Arquitectura decomposed para CPIT/PCPSP | Introducir una capa de descomposición que separe selección temporal, ruteo por destino y stockpiles, permitiendo bounds o warm starts desde relajaciones antes de aspirar a un solver industrial. Debe tomar como guía [R22], [R23], [R24], [R34], [R35] y [R36], dejando límites claros del MVP vs etapa avanzada. | Decomposed scheduling architecture | MR-176, MR-178, MR-179 | Existe una ruta reproducible que entrega candidato + bound/relajación y escala mejor que la baseline exacta sin perder auditabilidad ni acoplarse a Marvin. |
| 181 | MR-181 | `[ ]` | P1 | Benchmarks | Validación multi-mine del scheduler | Consolidar una matriz de verificación sobre Marvin y al menos otra instancia abierta (MineLib/Newman u otra equivalente) para asegurar que el scheduler usa el mismo contrato genérico y no supuestos Marvin-específicos. [R29] debe ser la referencia mínima para el set abierto. | Multi-mine scheduling benchmark | MR-167, MR-175, MR-177 | El benchmark corre sobre más de una mina/instancia con la misma API del core y reporta diferencias por dataset sin ramas especiales por nombre de mina. |
