...
| 133 | MR-133 | `[x]` | P1 | Benchmarks | Comparador de resultados benchmark | Construir comparadores reproducibles entre outputs Marvin externos y outputs generados por `mine-rs` para conteos, membresía de bloques, aristas de precedencia, tonelaje/metal y métricas derivadas. Existe reporte versionado en `datasets/benchmarks/marvin/outputs/comparison-report.json` con `prec` edge_jaccard=1.0, `upit` membership_jaccard=0.6458 y auditorías reproducibles de `cpit` / `pcpsp` / LP sobre las referencias locales; las comparaciones contra scheduling propio quedan pendientes de MR-171 y MR-172. | Benchmark comparator | MR-130, MR-132 | El comparador produce reportes serializables con coincidencias, diferencias y tolerancias explícitas por instancia. |
...
| 173 | MR-173 | `[ ]` | P1 | Planning | Backend exacto UPL escalable | Reemplazar o complementar el backend Edmonds-Karp actual por un algoritmo escalable (push-relabel, pseudoflow o equivalente) para que el solver exacto de UPL pueda correr sobre instancias tipo Marvin sin quedar restringido a fixtures pequeños. | Exact UPL scalable backend | MR-156 | Existe un backend exacto que mantiene paridad con fixtures pequeños actuales y resuelve Marvin/local benchmarks en tiempos razonables para comparación reproducible. |
| 174 | MR-174 | `[ ]` | P1 | Benchmarks | Comparar UPL exacto contra UPIT Marvin | Conectar el backend exacto escalable al benchmark Marvin usando el objetivo económico correcto por bloque para comparar memberships, tonelaje y valor contra `marvin_upit.sol`. | Exact UPL vs Marvin report | MR-170, MR-173 | `datasets/benchmarks/marvin/outputs/comparison-report.json` incluye una comparación exacta `mine-rs` vs UPIT Marvin, sustituyendo o complementando el prototipo heurístico actual. |
